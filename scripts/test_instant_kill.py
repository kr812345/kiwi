...
async def test_instant_kill():
    ...
    send_payload = {
        "type": "chat.message",
        "conversation_id": "test-instant-kill",
    ...
    try:
        while True:
            ...
            if mtype == "chat.stream":
                token_count += 1
                if token_count == 2 and not killed:
                    print(f"--> Instantly SIGKILLing kiwi-brain (PID {brain_pid})...")
        ...
        print("--> TIMEOUT! No message received for 5 seconds after brain killed!")
    finally:
        await ws.close()
        print("Restoring kiwi-brain via pm2 restart kiwi-brain...")
        subprocess.run(["pm2", "restart", "kiwi-brain"], check=True)

if __name__ == "__main__":
    asyncio.run(test_instant_kill())
